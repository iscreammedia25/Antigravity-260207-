...
// 1. Fix the container structure in prototype.html
// Wrap the static steps in a container that renderReadPhase can target
const oldPhaseReadStart = '<div id="phase-read" class="w-full h-full flex flex-col relative overflow-hidden hidden">';
const newPhaseReadStart = oldPhaseReadStart + '\n                <!-- Background Layer: Cover Image (Darkened) -->\n                <div id="read-bg" class="absolute inset-0 bg-cover bg-center transition-all duration-1000"\n                    style="filter: brightness(0.3) blur(8px)">\n                </div>\n                <div id="phaseContainer" class="flex-1 flex flex-col relative z-10">';

// We need to find where phase-read ends to close the new div.
// But it's easier to just replace the whole section since it's quite repetitive.
// Let's replace the static part with a cleaner version.

const phaseReadSectionRegex = /<!-- Read Phase - 3 Steps Flow -->[\s\S]*?<!-- Speak Phase -->/;
const newPhaseReadContent = `<!-- Read Phase - 3 Steps Flow -->
...
content = content.replace(phaseReadSectionRegex, newPhaseReadContent);

// 2. Update renderReadPhase to match user's preferred dark style for selection
// And ensure it sets the background image
const renderReadPhaseRegex = /function renderReadPhase\(\) \{[\s\S]*?if \(currentReadStep === 'intro'\)/;
const newRenderReadPhaseStart = `function renderReadPhase() {
            ...
            if (bgElement) {
                bgElement.style.backgroundImage = \`url('\${book.cover_url}')\`;
            }
...
content = content.replace(renderReadPhaseRegex, newRenderReadPhaseStart);

// 3. Update setPhase to call renderReadPhase
const setPhaseRegex = /document.getElementById\(\`phase-\$\{phase\}\`\)\.classList\.remove\('hidden'\);/;
content = content.replace(setPhaseRegex, "document.getElementById(`phase-${phase}`).classList.remove('hidden');\n            if (phase === 'read') renderReadPhase();");

fs.writeFileSync(filePath, content);
console.log('Fixed Read phase container, background, and dynamic labels in prototype.html');
